...
datrvsp <- regdat.d %>% filter(complete.cases(!!as.symbol(variablenames[3])))
...
for(n in 1:length(allid)){
  ...
  nZid3[[n]] <- nzid3 <- datrvsp %>% filter(pid == allid[n]) %>% select(REcolnames) %>% as.matrix
  ...
  nXid3[[n]] <- x3 <- datrvsp %>% filter(pid == allid[n]) %>% select(FEcolnames) %>% as.matrix
  ...
  nYi3[[n]] <- y3 <- datrvsp %>% filter(pid == allid[n]) %>% select(variablenames[3]) %>% as.matrix %>% set_colnames("y")
...
uvecRVSP <- list(U1vec, U2vec, U3vec)

par(mfrow = c(1, 3))
qqnorm(uvecRVSP[[1]], pch = 20, main = "Normal Q-Q Plot for pFVC scaled residuals"); abline(0, 1, col = "darkred")
qqnorm(uvecRVSP[[2]], pch = 20, main = "Normal Q-Q Plot for pDLCO scaled residuals"); abline(0, 1, col = "darkred")
qqnorm(uvecRVSP[[3]], pch = 20, main = "Normal Q-Q Plot for RVSP scaled residuals"); abline(0, 1, col = "darkred")
...
predictRVSPtraj <- function(G2, R2, beta, nZid1, nZid2, nZid3, nXid1, nXid2, nXid3, nYi1, nYi2, nYi3){
  ...
  for(n in 1:length(pid.cut)){
    ...
    for(j in 1:length(rvsptimes)){
      ...
      nrvsp3 <- sum(predtime - 3 <= z3[, 2] & z3[, 2] < predtime)
      ...
      newdfj <- data.frame(pid = pid.cut[n], predtime = predtime, rvspobs = rvspobs,
                           ...
                           p45 = pnorm(q = rvsp45, mean =  mu[3], sd = sqrt(sigma[3, 3])),
                           p50 = pnorm(q = rvsp50, mean =  mu[3], sd = sqrt(sigma[3, 3])),
                           nfvc = rowlist[1], ndlco = rowlist[2], nrvsp = rowlist[3],
                           nfvc3 = nfvc3, ndlco3 = ndlco3, nrvsp3 = nrvsp3)
...
for (k in 1:K){
  ...
  if (!file.exists(paste0("rvspfvcdlco", k,".Rdata"))){
    ...
    save(combmcmc, file = paste0("rvspfvcdlco", k,".Rdata"))
    
  }else{
    
    load(paste0("rvspfvcdlco", k,".Rdata"))
